...
module_section extern "C" struct {
    ...
    estd::uint8_t skip_4[64];
    // estd::uint8_t reserved_4[this_module_section_size - cleanup_offset - sizeof(exit)];
} __attribute__((__packed__)) __this_module = { .name = "iDriver", .init = init_module, .exit = cleanup_module };

// module_section extern "C" struct { // total: 896 bytes
//     estd::uint8_t skip_1[24];
//     char name[64 - sizeof(unsigned long)]; // 56 для 64-x бит
//     estd::uint8_t skip_2[232];
//     int (*init)(void); // Startup function
//     estd::uint8_t skip_3[504];
//     void (*exit)(void); // Destruction function
//     estd::uint8_t skip_4[64];
